...
class MaskRCNN(nn.Module):
    ...
    def build(self, config):
        ...
        ## @ Feature Pyramid Network FPN Classifier
        self.debug = False
    ...
    def predict(self, input, mode, use_nms=1, use_refinement=False, return_feature_map=False):
        ...
        # @ When mode is training (requirements are class id, boxes, masks, parameters) 
        ...
        # What is input - list?
            ...
            ## @ Normalize coordinates and scaling the coords
            h, w = self.config.IMAGE_SHAPE[:2]
            scale = Variable(torch.from_numpy(np.array([h, w, h, w])).float(), requires_grad=False)
            if self.config.GPU_COUNT:
                scale = scale.cuda()
